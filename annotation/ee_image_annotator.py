...
from utils.interpret import task2_interpretor, task3_interpretor, annotation_parser_prompt1_annotator1, annotation_parser_prompt4_annotator
...
class DataProducer(Process):
    ...
    def __init__(self, 
        ...
        self.metadata_db = metadata_db if metadata_db.startswith('/') else os.path.join(db_root, metadata_db)
        self.osm_db = osm_db if osm_db.startswith('/') else os.path.join(db_root, osm_db)
        self.annotation_db = annotation_db if annotation_db.startswith('/') else os.path.join(db_root, annotation_db)
        self.osm_wiki_db = osm_wiki_db if osm_wiki_db.startswith('/') else os.path.join(db_root, osm_wiki_db)
    ...
    def prefetch_metadata(self):
        """
        Fetch metadata from the patch table in the metadata database. and put them into a list.
        """
        
        c_meta = self.conn_metadata.cursor()
        c_meta.execute(self._SQL_QUERY_METAS, (self.map_element_threshold, self.prefetch_size))
        metas = c_meta.fetchall()
        self.logger.info(f'Fetched {len(metas)} metadata')
        return metas
    ...
    def task_selector(self, image_name, patch_name, patch_id):
        ...
        # query the osm statistics from the osm database
        c_osm_stat = self.conn_osm.cursor()
        c_osm_stat.execute(self._SQL_QUERY_OSM_STATISTICS, ('/'.join([image_name, patch_name]), ))
        osm_stat = c_osm_stat.fetchone()
        if osm_stat is None:
            # if there is no osm statistics, skip this task
            self.logger.error(f'No osm statistics for {image_name}/{patch_name}, skip this task. Patch id: {patch_id}.')
            return None
        bg_cnt, area_cnt, none_area_cnt = osm_stat[1:]
        
        if bg_cnt + area_cnt + none_area_cnt < 1:
            # if there is no osm data, skip this task
            self.logger.error(f'No osm data for {image_name}/{patch_name}, skip this task. Patch id: {patch_id}.')
            return None
        ...
        if len(available_task_types) < 1:
            # if there is no osm data, skip this task
            self.logger.error(f'No suitable task type for {image_name}/{patch_name}, skip this patch. Patch id: {patch_id}.')
            return None
    ...
    def prompt_selector(self, task_type):
        ...
        c.execute("SELECT ID, PROMPT FROM prompt WHERE TYPE =?", (task_type,))
        prompt_templates = c.fetchall()
        if len(prompt_templates) < 1:
            # if there is no prompt template, skip this task
            self.logger.error(f'No prompt template for task {task_type}, skip this task')
            return None
        seleted_prompt_template = random.choice(prompt_templates)
        prompt_id, prompt_template = seleted_prompt_template
        # self.logger.info(f'Selected prompt template {prompt_id} for task {task_type}')
        # fetch the valid annotator ids for this prompt
        c.execute("SELECT ANNOTATOR FROM annotator_prompt WHERE PROMPT =?", (prompt_id,))
        annotator_ids = c.fetchall()
        if len(annotator_ids) < 1:
            # if there is no valid annotator, skip this task
            self.logger.error(f'No valid annotator for prompt {prompt_id}, skip this task')
            return None
        
        annotator_ids = [i[0] for i in annotator_ids]
        
        return prompt_id, prompt_template, annotator_ids
    
    def generate_task(self, meta_data):
        
        # unpack the meta data
        image_name, patch_name = meta_data[:2]
        patch_id = meta_data[-1]
        ...
        c_osm.execute(self._SQL_QUERY_OSM.format(','.join([str(i) for i in osm_data_type])), 
                      ('/'.join([image_name, patch_name]), ))

        osm_elements = c_osm.fetchall()
        self.logger.info(f'Fetched {len(osm_elements)} osm elements for {image_name}/{patch_name}. Patch id: {patch_id}.')
        ...
        try:
            interpretation = self.taskx_interpreter[task_type](meta_data[:-1], osm_elements, self.osm_wiki_db, prompt_template, self.policy_config)
        ...
        if interpretation is None:
            ...
            self.logger.error(f'No suitable interpretation for {meta_data}, skip this task. Patch id: {patch_id}.')
            return None
        ...
        task = {'type': 'annotation_task',
                'patch_id': int(meta_data[-1]),
                'prompt_id': prompt_id,
                'valid_annotator_ids':annotator_ids}
        
        task.update(interpretation)
        # self.logger.info(f'1 patch id: {task["patch_id"]}')
        return task
    ...
    def run(self):
        ...
        metadata_path = self.metadata_db
        osm_path = self.osm_db
        annotation_path = self.annotation_db
        
        self.conn_metadata = sqlite3.connect(metadata_path, timeout=60)
        self.conn_osm = sqlite3.connect(osm_path, timeout=60)
        self.conn_annotation = sqlite3.connect(annotation_path, timeout=60)
        self.logger.info('Connected to the database')
        while not self.exit_flag.is_set():
            if len(self.meta_datas) < 1:
                self.meta_datas = self.prefetch_metadata()
            
            if len(self.meta_datas) < 1:
                # in this case, there is no more metadata to fetch, log and exit
                self.logger.info('No more metadata to fetch, exit')
                self.exit_flag.set()
                time.sleep(10)
                continue
            
            # get a task
            task = self.generate_task(self.meta_datas.pop(0))
            if task is None:
                # if there is no task, skip this iteration
                continue
            
            # send the task to the scheduler
            self.logger.debug(f'Send task to the manager')
            while not self.lock.acquire(blocking=False):
                self.logger.debug('Waiting for the lock')
                if self.exit_flag.is_set():
                    self.logger.debug('Exit flag is set, exit')
            ...
            while self.queue.full():
                self.logger.debug('Waiting for the queue to be available')
                if self.exit_flag.is_set():
                    self.logger.debug('Exit flag is set, exit')
                    self.lock.release()
                    self.disconnect_from_db()
                    return
                time.sleep(self.tick_interval)
            # self.logger.info(f'2 patch id: {task["patch_id"]}')
            self.queue.put(task)
            self.logger.debug('Task sent to the queue')
            self.lock.release()
    ...
    def stop(self) -> None:
        self.logger.info('Closing the data producer')
        self.exit_flag.set()
        self.logger.info('Closed the data producer')
    ...
    def run(self):
        ...
        while not self.exit_flag.is_set():
            ...
            if task['type'] =='annotation_response':
                patch_id, annotation, osm_ids, annotator_id, prompt_id = task['patch_id'], task['annotation'], task['osm_ids'], task['annotator_id'], task['prompt_id']
                ...
                try:
                    ...
                    self.logger.error(f'Error in parsing the annotation:\n {annotation} \n {e}')
                    continue
                
                # save the annotation to the database
                c = self.conn_annotation.cursor()
                c.execute("INSERT INTO annotation (PATCH, ANNOTATION, NUM_ELEMS, ANNOTATOR, PROMPT) VALUES (?,?,?,?,?)", 
                          (patch_id, annotation, len(osm_ids), annotator_id, prompt_id))
                annotation_id = c.lastrowid
                
                for osm_id in task['osm_ids']:
                    c.execute("INSERT INTO annotation_osm (ANNOTATION, OSM_ID) VALUES (?,?)", (annotation_id, osm_id))
                
                # add a annotation count to the patch table, and set to 1 if it is null
                c = self.conn_metadata.cursor()
                c.execute("UPDATE patch SET NUM_ANNOTATIONS = (COALESCE(NUM_ANNOTATIONS, 0) + 1) WHERE ID = ?", (task['patch_id'],))
                self.conn_annotation.commit()
                self.conn_metadata.commit()
                self.logger.info(f'Saved annotation {annotation_id} for patch {task["patch_id"]}, prompt_id {prompt_id}, annotator_id {annotator_id}')
...
class ManagerProcess(Thread):
    ...
    def __init__(self, db_root, 
        ...
        self.data_producers = [DataProducer(self.task_queue, 
                                            self.task_lock, 
                                            self.affair_queue, 
                                            self.affair_lock, 
                                            self.db_root, 
                                            ...
                                            self.tick_interval)for i in range(self.num_data_producer)]
        
        rewrite_producer = RewriteDataProducer(self.task_queue, 
                                                       self.task_lock, 
                                                       self.affair_queue, 
                                                       self.affair_lock, 
                                                       self.db_root, 
        ...
        self.data_worker = DataSaver(self.save_queue, 
                                      self.db_root, 
                                      self.metadata_db, 
                                      self.annotation_db, 
                                      self.tick_interval)
    ...
    def add_annotator(self, host, port):
        ...
        annotator_lock = Lock()
        annotator_connector = AnnotatorConnector(host,
                                                 port,
                                                 annotator_queue,
                                                 annotator_lock,
                                                 self.save_queue,
        ...
        self.annotator_connections[annotator_id].append({'dest_addr': f'{host}:{port}', 
            ...
            if not self.task_queue.empty():
                ...
                if task['type'] == 'annotation_task':
                    ...
                    if len(valid_annotator_ids) == 0:
                        self.logger.info(f'No valid annotator connection for task {task}')
                        continue
                    ...
                    for valid_annotator_id in valid_annotator_ids:
                        self.annotator_connections[valid_annotator_id] = sorted(self.annotator_connections[valid_annotator_id], key=lambda x: x['queue'].qsize())
                    
                    sorted_annotator_ids = sorted(valid_annotator_ids, key=lambda x: self.annotator_connections[x][0]['queue'].qsize())
                    
                    task['type'] = 'annotation_request'
                    self.logger.info(f'Distributing task with prompt_id {task["prompt_id"]} to annotators {sorted_annotator_ids}')
                    # aquire the lock to send the task to the annotator
                    self.annotator_connections[sorted_annotator_ids[0]][0]['queue'].put(task)
                    self.logger.info(f'Sent task to annotator {self.annotator_connections[sorted_annotator_ids[0]][0]["dest_addr"]}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--db_root', type=str, default='/media/slytheringe/Disk/Gejunyao/develop/VLP/testdownloading', help='Path to the data file')
    parser.add_argument('--metadata_db', type=str, default='metadata.db', help='the metadata sqilite database file name')
    parser.add_argument('--osm_db', type=str, default='osm.db', help='the osm sqilite database name')
    parser.add_argument('--annotation_db', type=str, default='annotation.db', help='the annotation sqilite database name')  
    parser.add_argument('--osm_wiki_db', type=str, default='taginfo-wiki.db', help='the osm wiki sqilite database path')  
    parser.add_argument('--log_file', type=str, default='annotation.log', help='the log file name')
    parser.add_argument('--fetch_size', type=int, default=100, help='the number of data to fetch each time')
    parser.add_argument('--tick_interval', type=float, default=0.01, help='the time interval to check the queue')
    parser.add_argument('--map_element_threshold', type=int, default=10, help='the threshold of the number of map elements')
    parser.add_argument('--num_data_producer', type=int, default=4, help='the number of data producer processes')
    parser.add_argument('--service_account', type=str, default='scriptlogin@ee-no-profit-osm.iam.gserviceaccount.com', help='The service account to use for GEE authentication.')
    parser.add_argument('--credentials_file', type=str, default='ee-no-profit-osm-ce98e252380f.json', help='The file to save the GEE credentials.')
    ...
    logging.basicConfig(handlers=[RotatingFileHandler(log_file, maxBytes=10000000, backupCount=9)],
                        level=logging.INFO, 
                        format=log_format)
    
    manager_process = ManagerProcess(db_root, 
                                      metadata_db, 
                                      osm_db, 
                                      annotation_db, 
                                      osm_wiki_db, 
