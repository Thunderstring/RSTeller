...
class DataProducer(Process):
    ...
    def __init__(
        ...
        self.logger = logging.getLogger(
            "[DataProducer{}]".format("" if producer_id is None else producer_id)
        )
    ...
    def prefetch_metadata(self):
        ...
        c_meta.execute(
            self._SQL_QUERY_METAS, (self.map_element_threshold, self.prefetch_size)
        )
    ...
    def task_selector(self, image_name, patch_name, patch_id):
        ...
        if osm_stat is None:
            ...
            self.logger.error(
                f"No osm statistics for {image_name}/{patch_name}, skip this task. Patch id: {patch_id}."
            )
            return None
    ...
    def prompt_selector(self, task_type):
        ...
        c.execute(
            "SELECT ANNOTATOR FROM annotator_prompt WHERE PROMPT =?", (prompt_id,)
        )
